...
<body>
					...
					<div class="review_box">
						...
						<div class="short_review_area">
							<div class="grade_area">
								...
								</span> <strong class="text_value"> <span><c:out
																...
																<div class="thumb_area">
																	<c:forEach var="commentImages"
																		...
																		<a href="" class="thumb" title="이미지 크게 보기"> <img
																			...
																			src="./<c:out value="${commentImages.saveFileName} "/>"
																			alt="리뷰이미지" />
																		</a>
																	</c:forEach>
															...
															<div class="info_area">
																<div class="review_info">
																	<span class="grade">${commentsList.score }</span> <span
																		...
																		class="date">${commentsList.reservationDate}</span>
																</div>
															</div>
														</div>
													</li>
															...
															<div class="info_area">
																<div class="review_info">
																	<span class="grade">${comments.score }</span> <span
																		...
																		class="date">${comments.reservationDate}</span>
																</div>
															</div>
														</div>
													</li>
														...
														<div>
															...
															<div class="info_area">
																<div class="review_info">
																	<span class="grade">${comments.score }</span> <span
																		class="name">${comments.reservationName }</span> <span
																		class="date">${comments.reservationDate }방문</span>
																</div>
															</div>
														</div>
													</li>
											...
											<div>
												<div class="review_area">
													...
													<p class="review">2층이어서 걱정했는데 꽤잘보여서 좋았습니다 고미오 너무 멋있었습니다
														사진은 커튼콜때 찍었습니다 끝나고 퇴근길도 봐서 너무 Dummy Data</p>
												</div>
												<div class="info_area">
													<div class="review_info">
					...
					<div class="detail_location hide">
						<div class="box_store_info no_topline">
							<a href="" class="store_location" title="지도웹으로 연결"> <img
								...
								src="${displayInfoImage.saveFileName }" /> <span
	...
	<script>
		...
		$(document).ready(function() {
			...
			$(".section_store_details>.bk_more").click(function() {
				var more = $(this);

				// submenu 가 화면상에 보일때는 위로 보드랍게 접고 아니면 아래로 보드랍게 펼치기
				if (more.hasClass("_open") === true) {
					var details = $(".store_details.close3");
					var close = more.next("a");
					more.hide();
					close.show();
					details.removeClass("close3");
				} else {
					var details = $(".store_details");
					var open = more.prev("a");
					more.hide();
					open.show();
					details.addClass("close3");
				}
			});
		});
		...
		reserveView_btn.addEventListener("click", function(evt) {
			...
			var numSpan = document.querySelector(".figure_pagination>.num:not(.off)")
			var imgCnt = 0;
			...
			if(imgCnt > 1){
				...
				prevBtn.addEventListener("click", function() {
					var curIndex = numSpan.innerText - 1;
			...
			function slideShow(curIndex) {
				if(curIndex === 0){
					image_ul.style.transition = "transform 0.1s ease-out";
			...
			infoTab.addEventListener("click", function(evt) {
				var clickEle = evt.target.tagName;
				var target = evt.target;
				var activeEle = document.querySelector('.anchor.active');
				var notactiveEle = document.querySelector('.anchor:not(.active)');
				var detail = document.querySelector('._detail');
				var path = document.querySelector('._path');
				var detail_area_wrap = document.querySelector('.detail_area_wrap');
				var detail_location = document.querySelector('.detail_location');
				
				var detailChild = detail.childNodes[0].classList;
				var pathChild = path.childNodes[0].classList;
				
				if(clickEle === 'LI'){
					if(!target.childNodes[0].classList.contains('active')){
						activeEle.classList.toggle('active');
						notactiveEle.classList.add('active');
						
						if(detailChild.contains('active')){
							detail_area_wrap.classList.remove('hide');
							detail_location.classList.remove('hide');
							detail_location.classList.add('hide')
						}else if(pathChild.contains('active')){
							detail_area_wrap.classList.remove('hide');
							detail_location.classList.remove('hide');
